...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<FIPClassify> classi(new FIPClassify(*ip, 1, 4));
    if (!classi->init()) {
    ...
    std::shared_ptr<MultiOSGConsumer> osgc(new MultiOSGConsumer(*classi, 1, 4));
    if (!osgc->init()) {
...
*/
    ...
    /*{ //
        ...
        translate.makeTranslate(osg::Vec3d(osgc->getOutputTexture()->getTextureWidth(),  //getTextureWidth/2
                                           0.0,
                                           0.0));
...
*/
    ...
    size_t numFrames=0;
    while((!viewer.done()) && (numFrames<20) ) /*&&(ffp->getCurrentImage()<(ffp->getNumImages()*0.9f)) )*/
    {
        ...
        {
            FThread::microSleep(1000);  //@20000 worked
        }
    }
