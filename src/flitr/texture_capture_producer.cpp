...
TextureCaptureProducer::TextureCaptureProducer(
...
void TextureCaptureProducer::createTextures()
{
    ...
    if (OutputFormat_.getPixelFormat() == ImageFormat::FLITR_PIX_FMT_Y_8) {
        OutputTexture_->setInternalFormat(GL_LUMINANCE);
        ...
        OutputTexture_->setSourceFormat(GL_RGB);
        OutputTexture_->setSourceType(GL_UNSIGNED_BYTE);
    }
...
void TextureCaptureProducer::setupCamera()
{
    ...
    Camera_->setPostDrawCallback(new TextureCaptureDrawCallback(this));
}
