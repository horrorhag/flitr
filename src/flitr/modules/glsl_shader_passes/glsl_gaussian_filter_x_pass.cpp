    ...
    Title_("GaussianFilterX")
{
    ...
    Kernel1DImage_->allocateImage(TextureWidth_, 1, 1,
                                  GL_RED, GL_FLOAT);
...
GLSLGaussianFilterXPass::~GLSLGaussianFilterXPass()
{
}

osg::ref_ptr<osg::Group> GLSLGaussianFilterXPass::createTexturedQuad()
{
...
void GLSLGaussianFilterXPass::setupCamera()
{
    // clearing
...
void GLSLGaussianFilterXPass::setShader()
{
    osg::ref_ptr<osg::Shader> fshader = new osg::Shader( osg::Shader::FRAGMENT,
                                                         ...
                                                         "    {\n"
                                                         "      kernelValue=texture2DRect(kernel1DTexture, vec2(float(r)+0.5, 0.5)).r;\n"
                                                         "      kernelValueSum+=kernelValue*2.0;\n"

                                                         "      filteredPixel+=(texture2DRect(textureID0, texCoord-vec2(float(r),0))+texture2DRect(textureID0, texCoord+vec2(float(r),0)))*kernelValue;\n"
                                                         "    }\n"
    ...
    FragmentProgram_->setName("glsl_gaussfilter_x");
...
void GLSLGaussianFilterXPass::setStandardDeviation(float sd)
{
