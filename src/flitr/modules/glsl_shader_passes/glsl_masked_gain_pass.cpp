    ...
    Title_(std::string("Masked Gain"))
{
    ...
    ImageTexture_ = in_tex;
    MaskTexture_ = mask_tex;
    OutTexture_=0;
    OutImage_=0;


    GainUniform_=osg::ref_ptr<osg::Uniform>( new osg::Uniform("gain", (float)1.0f) );
...
GLSLMaskedGainPass::~GLSLMaskedGainPass()
{
}

osg::ref_ptr<osg::Group> GLSLMaskedGainPass::createTexturedQuad()
{
    ...
    StateSet_->setTextureAttributeAndModes(0, ImageTexture_.get(), osg::StateAttribute::ON);
    StateSet_->addUniform(new osg::Uniform("textureID0", 0));

    StateSet_->setTextureAttributeAndModes(1, MaskTexture_.get(), osg::StateAttribute::ON);
    StateSet_->addUniform(new osg::Uniform("textureID1", 1));

    StateSet_->addUniform(GainUniform_);
...
void GLSLMaskedGainPass::setupCamera()
{
    // clearing
...
void GLSLMaskedGainPass::createOutputTexture(bool read_back_to_CPU)
{
    OutTexture_ = new flitr::TextureRectangle;

    OutTexture_->setTextureSize(TextureWidth_, TextureHeight_);
    OutTexture_->setInternalFormat(ImageTexture_->getInternalFormat());
    OutTexture_->setSourceFormat(ImageTexture_->getSourceFormat());
    OutTexture_->setSourceType(ImageTexture_->getSourceType());
    ...
    if (read_back_to_CPU)
    {
        ...
        OutImage_->allocateImage(TextureWidth_, TextureHeight_, 1,
                                 ImageTexture_->getInternalFormat(), ImageTexture_->getInternalFormatType());
    }
}

void GLSLMaskedGainPass::setShader()
{
    osg::ref_ptr<osg::Shader> fshader = new osg::Shader( osg::Shader::FRAGMENT,
                                                         ...
                                                         "    vec3 mask  = texture2DRect(textureID1, texCoord).rgb;\n"
                                                         "    gl_FragColor.rgb = (rgb - mask)*gain + mask;\n"
                                                         "    gl_FragColor.a = 1.0f;\n"
                                                         "}\n"
                                                         );
...
void GLSLMaskedGainPass::setGain(float value)
{
    GainUniform_->set(value);
}

float GLSLMaskedGainPass::getGain() const
{
    float rValue=1.0f;

    GainUniform_->get(rValue);
