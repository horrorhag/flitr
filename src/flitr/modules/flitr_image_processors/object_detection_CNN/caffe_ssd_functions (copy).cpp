...
void SingleShotDetector::SetMean(const string& mean_file, const string& mean_value) {
  ...
  if (!mean_file.empty()) {
    ...
    if(mean_blob.channels() != num_channels_)
        cerr<< "Number of channels of mean file doesn't match input layer." <<endl;
  ...
  if (!mean_value.empty()) {
    ...
    if(values.size() == (uint) 1 || values.size() == (uint) num_channels_)
        cerr<<"Specify either 1 mean_value or as many as channels: " << num_channels_ <<endl;
  ...
  // Process image one by one.
    ...
    for (uint i = 0; i < numOfDetectionResults; ++i) {
        ...
        if (score >= confidence_threshold) {
            ...
            std::cout << "Class: " << classId << " = " << className <<std::endl;

            std::cout << "i=" <<i << " Confidence: " << d[2] << std::endl;
            ...
            if(bestClassFound == false){
                cv::putText(image, className.c_str() , cv::Point(xLeftBottom,yLeftBottom-5),
                    cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(100,240,100),1.5, CV_AA);
